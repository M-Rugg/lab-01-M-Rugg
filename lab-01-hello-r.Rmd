...
There are **1846** rows with **3** variables. These variables are **dataset**, **x**, and **y**.
...
```{r fig.fullwidth=TRUE, fig.height=4, fig.width=5}
...
**r = -0.0629611** The `r` for `star` is just slightly larger than the the `r` for `dino`. However, they are both negative and within 0.0015 of each other.
...
**r = -0.06834336** The `r` of `circle` is slightly smaller than the `r` of `dino`. They are both negative.
...
```{r fig.fullwidth=TRUE, fig.height=12, fig.width=6}
